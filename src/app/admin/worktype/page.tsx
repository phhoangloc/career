...
const Page = () => {
    ...
    useEffect(() => {
        currentUser.position && getMedia(currentUser.position, "worktype", "", undefined, undefined)
    ...
    useEffect(() => {
        currentAlert.value && id && deleteImage(currentUser.position, "worktype", id)
    }, [currentAlert.value])
    ...
    const updateCategory = async (id: string, body: any) => {
        ...
        const result = await UserAuthen.updateItem(currentUser.position, "worktype", id, body)
        if (result.success) {
            setRefresh(n => n + 1)
            setIsEdit(false)
            setEditName("")
        ...
        <div style={{ minHeight: "calc(100vh - 70px)", width: "100%", maxWidth: "575px", margin: "5px" }}>
            <div className="dp-flex">
                {
                    isNew ?
                        <div style={{ display: "flex" }}>
                            <input style={{ borderRadius: "5px", fontSize: "1rem" }} placeholder='新規職種' onChange={(e) => { setName(e.target.value) }} />
            ...
            {data.length ? data.map((item, index) =>
                <div key={index} className='flexbox hover-background-color-128-15p hover-boder-radius-5px hover-opacity-1'
                    ...
                    {isEdit && id === item._id ?
                        <input style={{ border: "none", fontSize: "1rem", width: "100%", background: "none" }} placeholder='職種を入力'
                ...
                <div className='flexbox'>
                    ...
                    <div style={{ width: "100%", textAlign: "center" }}>{loading ? <p>少々お待ちください。</p> : <p>{"職種がありません。"}</p>}</div>
                    <div style={{ width: "50px" }}></div>
                </div>
            }
        </div>
