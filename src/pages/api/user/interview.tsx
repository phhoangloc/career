...
const Post =
    async (
        ...
        const post = await interviewModel.findOne({ "_id": query.id })
        ...
        if (id) {
            if (position === "admin") {
                switch (method) {
                    ...
                    case "POST":
                        body.host = id
                        await interviewModel.create(body)
                            .catch((error: Error) => {
                                result.success = false
                                result.message = error.message
                                res.send(result)
                    ...
                    case "PUT":
                        if (host.toString() === id.toString()) {
                            body.host = id
                            await interviewModel.updateOne({ "_id": query.id }, body)
                                .catch((error: Error) => {
                                    result.success = false
                                    result.message = error.message
                                    res.send(result)
                    ...
                    case "DELETE":
                        if (host.toString() === id.toString()) {
                            await interviewModel.deleteOne({ "_id": query.id })
                                .catch((error: Error) => {
                                    result.success = false
                                    result.message = error.message
                                    res.send(result)
